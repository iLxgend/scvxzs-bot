...
export default class MemeCommand extends BaseCommand {
    ...
    public async process(msg: string, answer: IBotMessage, message: discord.Message, client: discord.Client, config: IBotConfig, commands: IBotCommand[]): Promise<void> {
        ...
        if (splitted.length > 1) {
            ...
            // Search on Reddit
            snekfetch.get(`https://www.reddit.com/search/.json?q='${query}'&sort=top&t=day&limit=350`).then(res => {
    ...
    private sendEmbed(res: any, message: discord.Message, query: string | null) {
        ...
        // Get random meme
        let post = res.body.data.children.filter(child =>
            child.data.title != null &&
            ((child.data.url as string).endsWith(".jpg")
                || (child.data.url as string).endsWith(".png")
                || (child.data.url as string).endsWith(".gif")))[Math.floor(Math.random() * res.body.data.children.length)] // :v

        if (!post) post = res.body.data.children[Math.floor(Math.random() * res.body.data.children.length)];
        ...
        if ((post.data.url as string).endsWith(".jpg")
            ...
            memeEmbed.addField("Image", "Couldn't find an image, so here's the link to the post/reaction.")
        }
